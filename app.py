...
if not uploaded_file or not uploaded_blacklist:
    ...
    blocked_mask = df_key_company.isin(blacklist_companies) | df_key_email.isin(blacklist_emails)
    df_blocked = df[blocked_mask].copy()
    df_clean = df[~blocked_mask].copy()
    ...
    # Initialize session state for live edits
